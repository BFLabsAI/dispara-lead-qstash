  ...
  return (
    <Card className={`rounded-2xl card-premium animate-slide-in-up mb-8 ${isDark ? 'glass-card' : 'bg-gradient-to-br from-green-500 to-emerald-600 shadow-2xl border-0'}`}>
      <CardContent className={`p-6 ${isDark ? '' : 'text-white'}`}>
        <div className="mb-4">
          <div className={`w-12 h-1 rounded-full mb-2 ${isDark ? 'bg-gradient-to-r from-green-500 to-emerald-600' : 'bg-white/30'}`}></div>
        ...
        <div className="flex flex-wrap gap-3 items-end"> {/* Layout horizontal: 3 filtros + X; gap-3, items-end para alinhar */}
          ...
          <div className="flex-1 min-w-[220px] space-y-1">
            ...
            <Select value={instance} onValueChange={(value) => { setInstance(value); }}>
              <SelectTrigger className={`glass-card h-10 ${isDark ? 'border-white/20 bg-black/20 text-white hover:bg-green-500/10' : 'bg-white/20 backdrop-blur-sm border-white/30 text-white hover:bg-white/30'}`}>
