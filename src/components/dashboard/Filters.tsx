      ...
      <CardContent className={`p-6 ${isDark ? 'text-white' : 'text-gray-900'}`}>
        <div className="mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-lg text-green-600">Filtros Avançados</h3>
          </div>
        </div>
        <div className="flex flex-wrap gap-3 items-end">
          ...
          <div className="flex-1 min-w-[220px] space-y-1">
            <Label className="flex items-center gap-1 text-sm font-medium text-gray-700">
              <i className="fas fa-server text-xs text-green-600"></i> Instância
            </Label>
            <Select value={instance} onValueChange={(value) => { setInstance(value); }}>
              <SelectTrigger className="glass-card h-10 bg-white/20 backdrop-blur-sm border-green-200 text-gray-800 hover:bg-white/30">
                <SelectValue placeholder="Todas" />
              </SelectTrigger>
              <SelectContent className="glass-card max-h-48 bg-white border-green-200 text-gray-800">
                <SelectItem value="all" className="text-gray-800 hover:bg-green-50">Todas</SelectItem>
              </SelectContent>
            </Select>
          </div>
          ...
          <div className="flex-1 min-w-[220px] space-y-1">
            <Label className="flex items-center gap-1 text-sm font-medium text-gray-700">
              <i className="fas fa-tag text-xs text-green-600"></i> Tipo
            </Label>
            <Select value={tipo} onValueChange={(value) => { setTipo(value); }}>
              ...
              <SelectContent className="glass-card max-h-48 bg-white border-green-200 text-gray-800">
                <SelectItem value="all" className="text-gray-800 hover:bg-green-50">Todos</SelectItem>
                <SelectItem value="texto" className="text-gray-800 hover:bg-green-50">Texto</SelectItem>
                <SelectItem value="imagem" className="text-gray-800 hover:bg-green-50">Imagem</SelectItem>
              </SelectContent>
            </Select>
          </div>
          ...
          <div className="flex-1 min-w-[220px] space-y-1">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" className="glass-card w-full justify-start text-left h-10 bg-white/20 backdrop-blur-sm border-green-200 text-gray-800 hover:bg-white/30">
                  {getDateDisplay(dateRange)}
                </Button>
              </PopoverTrigger>
              <PopoverContent className="glass-card w-auto p-0 bg-white border-green-200 text-gray-800" align="start">
