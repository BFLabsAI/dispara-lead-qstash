...
export const Filters = ({ onFilterChange }: FiltersProps) => {
  ...
  const getDateDisplay = (range: DateRange | undefined) => {
    if (!range?.from) return <span className={isDark ? 'text-green-300' : 'text-white/80'}>Selecionar período</span>; // Placeholder green no dark
  ...
  return (
    <Card className={`rounded-2xl card-premium animate-slide-in-up mb-8 ${isDark ? 'glass-card bg-green-900/10 border-green-500/20' : 'bg-gradient-to-br from-green-500 to-emerald-600 shadow-2xl border-0'}`}> {/* Dark: subtle green bg/border para manter cor */}
      <CardContent className={`p-6 ${isDark ? '' : 'text-white'}`}>
        <div className="mb-4">
          <div className={`w-12 h-1 rounded-full mb-2 ${isDark ? 'bg-gradient-to-r from-green-500/50 to-emerald-600/50' : 'bg-white/30'}`}></div> {/* Top bar green sutil no dark */}
          <div className="flex items-center gap-2">
            <Filter className={`h-4 w-4 ${isDark ? 'text-green-400' : 'text-white'}`} />
            <h3 className={`font-bold text-lg ${isDark ? 'gradient-text' : 'text-white drop-shadow-md'}`}>Filtros Avançados</h3>
          </div>
        </div>
        <div className="flex flex-wrap gap-3 items-end"> {/* Layout horizontal: 3 filtros + X; gap-3, items-end para alinhar */}
          ...
          <div className="flex-1 min-w-[220px] space-y-1">
            <Label className={`flex items-center gap-1 text-sm font-medium ${isDark ? 'text-green-300' : 'text-white/90'}`}> {/* Label green no dark */}
            ...
            <Select value={instance} onValueChange={(value) => { setInstance(value); }}>
              <SelectTrigger className={`glass-card h-10 ${isDark ? 'border-green-500/20 bg-black/20 text-white hover:bg-green-500/20' : 'bg-white/20 backdrop-blur-sm border-white/30 text-white hover:bg-white/30'}`}> {/* Hover green mais visível no dark */}
                <SelectValue placeholder="Todas" />
              </SelectTrigger>
              <SelectContent className={`glass-card max-h-48 ${isDark ? 'bg-black/20 border-green-500/20 text-white' : 'bg-white/90 border-green-200 text-gray-900'}`}> {/* Content green border no dark */}
          ...
          <div className="flex-1 min-w-[220px] space-y-1">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" className={`glass-card w-full justify-start text-left h-10 ${isDark ? 'border-green-500/20 bg-black/20 text-white hover:bg-green-500/20' : 'bg-white/20 backdrop-blur-sm border-white/30 text-white hover:bg-white/30'}`}> {/* Hover green mais visível no dark */}
                  {getDateDisplay(dateRange)}
                </Button>
              </PopoverTrigger>
              <PopoverContent className={`glass-card w-auto p-0 ${isDark ? 'border-green-500/20 bg-black/20 max-w-none text-white' : 'bg-white border-green-200 text-gray-900'}`} align="start"> {/* Content green border no dark */}
