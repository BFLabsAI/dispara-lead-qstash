# SQL Editor Runbook - Phase 2

- Timestamp: `2026-03-12`
- Scope:
  - contador atômico por campanha
  - circuit breaker por tenant
  - materialized view diária para stats
  - refresh RPC da materialized view
...
DROP MATERIALIZED VIEW IF EXISTS public.dashboard_daily_stats_mv;

CREATE MATERIALIZED VIEW public.dashboard_daily_stats_mv AS
SELECT
  tenant_id,
  ...
  COUNT(*) FILTER (WHERE COALESCE((metadata->>'usaria')::boolean, false) = true)::bigint AS total_ia
FROM public.message_logs_dispara_lead_saas_03
...
## 2. Depois do SQL, rode no terminal
...
## 3. Depois do deploy, rode um refresh inicial da MV

```sql
select public.refresh_dashboard_daily_stats_mv();
```

## 4. Validações esperadas

- `campaigns_dispara_lead_saas_02` passa a ter `sent_count` e `failed_count`
- `process-message` e `process-message-ai` continuam respondendo
- breaker abre só após `3` falhas consecutivas por tenant
- campanha do tenant afetado muda para `paused` quando o breaker abre
- `get_dashboard_stats` passa a ler da `dashboard_daily_stats_mv`
